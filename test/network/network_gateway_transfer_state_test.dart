...
void main() {
  group('NetworkGateway transfer state bounds', () {
    test('cancels tracked dio transfer without probing rust in v1', () async {
    ...
    test('polls only dio transfer events in v1', () async {
      var dioPollCalls = 0;
      var rustPollCalls = 0;
...
class _FakeTransferAdapter implements NetAdapter {
  ...
  _FakeTransferAdapter({
    required this._startTransferDelegate,
    Future<List<NetTransferEvent>> Function({int limit})? pollTransferDelegate,
    required this._cancelTransferDelegate,
