...
#[tokio::test]
async fn root_byte_budget_prunes_across_namespaces() {
    let cache_dir = create_test_cache_dir_path("root_budget");
    let seed_cache = DiskCache::new_for_test(
        cache_dir.to_string_lossy().as_ref(),
        Duration::from_secs(300),
        8,
        None,
    ...
    seed_cache
        ...
        .await
        .expect("store ns_a k1");
    let max_root_bytes = total_file_bytes(&cache_dir).await;
    tokio::time::sleep(Duration::from_millis(4)).await;
    seed_cache
        .store(
            "ns_b",
    ...
    .expect("create cache with root budget");
    cache.prune_root().await.expect("prune root");

    let ns_a = cache.lookup("ns_a", "k1").await.expect("lookup ns_a");
    let ns_b = cache.lookup("ns_b", "k2").await.expect("lookup ns_b");
...
#[tokio::test]
async fn root_byte_budget_counts_metadata_toward_actual_disk_usage() {
    let cache_dir = create_test_cache_dir_path("root_budget_metadata");
    let seed_cache = create_cache(&cache_dir, 1024 * 1024);
    let key =
        DiskCache::build_cache_key("GET", "https://example.com/root-budget-metadata", &[], None);
    ...
    seed_cache
        .store(
            ...
            &key,
            "GET",
            "https://example.com/root-budget-metadata",
            200,
            &[("cache-control".to_owned(), "max-age=120".to_owned())],
            CacheBodySource::Bytes(b"12345"),
        )
    ...
    assert!(
        actual_bytes > 5,
        "metadata should contribute to on-disk usage"
    );
    ...
    let cache = DiskCache::new_for_test(
        ...
        Duration::from_secs(300),
        1024 * 1024,
        Some(actual_bytes.saturating_sub(1)),
    )
    .expect("create cache with root budget");
    ...
    let cached = cache
        ...
        .expect("lookup cache");
    assert!(cached.is_none());
    assert!(total_file_bytes(&cache_dir).await <= actual_bytes.saturating_sub(1));
...
#[tokio::test]
async fn root_prune_removes_residual_files_under_cache_root() {
    let cache_dir = create_test_cache_dir_path("root_budget_residual");
    let namespace_dir = cache_dir.join(RESPONSE_CACHE_NAMESPACE);
    tokio::fs::create_dir_all(namespace_dir.join("nested"))
        .await
        .expect("create residual dir");
    tokio::fs::write(cache_dir.join("root.tmp"), b"12")
        .await
        .expect("write root residual");
    tokio::fs::write(namespace_dir.join("orphan.body.bin"), b"345")
        .await
        .expect("write orphan body");
    tokio::fs::write(namespace_dir.join("stray.txt"), b"6789")
        .await
        .expect("write stray file");
    tokio::fs::write(namespace_dir.join("nested").join("ghost.bin"), b"00")
        .await
        .expect("write nested ghost file");
    ...
    assert!(tokio::fs::metadata(cache_dir.join("root.tmp"))
        .await
        .is_err());
    assert!(tokio::fs::metadata(namespace_dir.join("orphan.body.bin"))
        .await
        .is_err());
    assert!(tokio::fs::metadata(namespace_dir.join("stray.txt"))
        .await
        .is_err());
    assert!(tokio::fs::metadata(namespace_dir.join("nested"))
        .await
        .is_err());
    assert_eq!(total_file_bytes(&cache_dir).await, 0);
...
#[tokio::test]
async fn lookup_waits_out_concurrent_root_prune_during_metadata_temp_write() {
    let cache_dir = create_test_cache_dir_path("root_budget_lookup_prune_race");
    let cache = DiskCache::new_for_test(
        cache_dir.to_string_lossy().as_ref(),
        Duration::from_secs(300),
        1024 * 1024,
    ...
    .expect("create cache with root budget");
    let key = DiskCache::build_cache_key("GET", "https://example.com/race", &[], None);
    ...
    let pause = cache
        .pause_next_temp_write_in_namespace(RESPONSE_CACHE_NAMESPACE)
        .expect("install temp-write pause");
    let lookup_cache = cache.clone();
    let lookup_key = key.clone();
    let lookup_task = tokio::spawn(async move {
        lookup_cache
            .lookup(RESPONSE_CACHE_NAMESPACE, &lookup_key)
            .await
    });

    pause.wait_until_ready().await;

    let prune_cache = cache.clone();
    let prune_task = tokio::spawn(async move { prune_cache.prune_root().await });
    tokio::time::sleep(Duration::from_millis(50)).await;
    assert!(
        !prune_task.is_finished(),
        "root prune should wait for the in-flight metadata temp write"
    );

    pause.resume();

    let cached = lookup_task
        .await
...
async fn total_file_bytes(root: &PathBuf) -> u64 {
    let mut total = 0_u64;
