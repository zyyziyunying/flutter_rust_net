...
5. 下一会话默认从“先落 Rust upload 分支 + Rust 单测”开始，完成后再进入 Dart 侧一致性测试；若性能结论变化再补 `test_run_log` 记录。
...
6. 落地 Wi-Fi/4G/弱网 + 远端链路补测并归档，若性能结论变化再回填 `docs/test_plans/test_run_log.md` 与策略文档，输出“业务 App 接入前”准入结论（继续 Rust 默认 / 临时回切 Dio）。
...
6. 若性能结论变化，最后回填 `test_run_log` 与策略文档，给出业务接入决策（Rust 默认或先回切 Dio）。
