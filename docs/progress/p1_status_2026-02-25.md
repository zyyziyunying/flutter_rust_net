...
7. 尝试按新口径重跑 `jitter` 基线时，发现 Rust init 异常：`PanicException(... UnexpectedEof ...)`（`native/rust/net_engine/src/frb_generated.rs:668` 触发），当前作为 P1 基线复跑前置阻塞项。

## 2) 当前正在做（In Progress）

1. 排查 Rust init `UnexpectedEof` 异常，恢复 `jitter` Rust 通道基线复跑能力。
2. 执行 Rust 配置项生效性验证（`write_timeout_ms`、`max_connections`、`max_connections_per_host`）。
3. 推进真机 JSON 归档链路闭环（上传入口与登录/`token` 头已打通，待补齐命名约定与服务端回执口径）。
4. 推进远端链路 benchmark 联调与稳定自动化命令固化（CLI `--base-url` 已开放，待形成固定脚本/样例）。

## 3) 下一步准备做（Next）

1. 先定位并修复 Rust init `UnexpectedEof`，恢复 Rust `jitter(c16,mif32)` 可稳定复跑。
2. 执行 Rust 配置项生效性验证（`write_timeout_ms`、`max_connections`），沉淀可复现结论。
3. 基于 `--base-url` 完成“真机 -> 公网服务”非 loopback benchmark 自动化命令固化。
4. 基于 `http://47.110.52.208:7777/upload` 固化 JSON 命名约定与服务端回执口径，形成可追溯归档规范。
5. 落地 Wi-Fi/4G/弱网 + 远端链路补测并归档，按需同步更新 `相关文档（按需）` 与策略文档，输出“业务 App 接入前”准入结论（继续 Rust 默认 / 临时回切 Dio）。

---

## 你现在只需要做的事（按顺序）

1. 先修复 Rust init `UnexpectedEof`，确保 Rust 通道 benchmark 可跑。
2. 校验 `write_timeout_ms`、`max_connections` 等配置项是否实际生效并记录结论。
3. 基于 `--base-url` 联调远端 benchmark 服务并固化自动化命令。
4. 基于已知上传入口固化命名约定与回执口径，执行真机弱网与远端链路补测并归档 JSON。
5. 最后按需同步更新对应专题文档与策略文档，给出业务接入决策（Rust 默认或先回切 Dio）。
