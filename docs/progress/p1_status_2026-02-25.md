...
>
> 当前状态（2026-03-11）：P1 的基准口径、upload 语义、`maxInFlightTasks=32`、Rust 配置项生效链路均已在代码与测试中闭环；最终业务接入准入结论仍未输出，剩余工作集中在非 loopback 证据归档，以及 transfer 状态边界/重复初始化配置一致性两个后续高优先级风险。
...
- 风险审查补充：[`flutter_rust_net/docs/questions/flutter_rust_net_risk_review_findings_2026-03-09.md`](../questions/flutter_rust_net_risk_review_findings_2026-03-09.md)
...
4. 收敛后续高优先级接入风险：transfer 事件/任务状态边界，以及 Rust 重复初始化的配置一致性保护。
...
4. 补 transfer 事件/任务状态边界与 Rust 重复初始化配置一致性保护，再输出“业务 App 接入前”准入结论（继续 Rust 默认 / 临时回切 Dio）。
