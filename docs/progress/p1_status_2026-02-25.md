...
1. 执行 Rust 配置项生效性验证（`write_timeout_ms`、`max_connections`、`max_connections_per_host`）。
2. 推进真机 JSON 归档链路闭环（上传入口与登录/`token` 头已打通，待补齐命名约定与服务端回执口径）。
3. 推进远端链路 benchmark 联调与稳定自动化命令固化（CLI `--base-url` 已开放，待形成固定脚本/样例）。

## 3) 下一步准备做（Next）

1. 执行 Rust 配置项生效性验证（`write_timeout_ms`、`max_connections`），沉淀可复现结论。
2. 基于 `--base-url` 完成“真机 -> 公网服务”非 loopback benchmark 自动化命令固化。
3. 基于 `http://47.110.52.208:7777/upload` 固化 JSON 命名约定与服务端回执口径，形成可追溯归档规范。
4. 落地 Wi-Fi/4G/弱网 + 远端链路补测并归档，按需同步更新 `相关文档（按需）` 与策略文档，输出“业务 App 接入前”准入结论（继续 Rust 默认 / 临时回切 Dio）。

---

## 你现在只需要做的事（按顺序）

1. 校验 `write_timeout_ms`、`max_connections` 等配置项是否实际生效并记录结论。
2. 基于 `--base-url` 联调远端 benchmark 服务并固化自动化命令。
3. 基于已知上传入口固化命名约定与回执口径，执行真机弱网与远端链路补测并归档 JSON。
